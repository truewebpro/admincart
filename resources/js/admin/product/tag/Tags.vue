...
export default {
    ...
    computed: {
        filteredTags() {
            // if (this.status === "All") return this.tags;
            if (this.status === "Archived") return this.tags.filter(p => p.tag_status === "Archived");
            // return this.tags.filter(p => p.product_status === this.status);
            return this.tags.filter((p) => {
                const matchStatus = this.status === "All" || p.tag_status === this.status;
                return matchStatus;
            });
        }
    },
