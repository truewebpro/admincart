<template>
    <v-container>
        ...
        <v-tabs v-model="btab" color="primary" selectedClass="bg-lblue"
                ...
                class="my-3">
            <v-tab value="general" class="bg-white">Brand</v-tab>
                ...
                <v-row>
                    ...
                    <v-col cols="12" md="3">
                        ...
                        <v-card class="border-sm mt-4">
                            ...
                            <v-card-text>
                                <v-img v-if="brand.brand_image != null && !newImage && !libraryImagePath"
                                       :src="cdn+brand.brand_image" max-height="200"></v-img>
...
export default {
    ...
    components:{MediaLibraryPicker, BrandSections, BrandFaqs, RichTextEditor, VFileUpload},
    data(){
        return{
            btab:'general',
            stypes:[],
       ...
       async getBrandById(){
        ...
        editBrand(){
            ...
            if (this.newImage instanceof File) {
                brandImage = this.newImage;
                ImageAlt = this.image_alt; // only meaningful for a fresh upload
            } else if (this.libraryImagePath) {
                brandImage = this.libraryImagePath;
            } else if (this.brand.brand_image) {
                brandImage =  this.brand.brand_image;
            }
            const ubrand = {
