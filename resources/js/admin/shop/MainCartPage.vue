<template>
    <v-container class="page homepage">
...
export default{
    ...
    methods:{
        async fetchCartpage() {
            try {
                const res = await axios.get("/sadmin/cartpage/default")
                this.hlayout = res.data.cartpage;
                this.sections = res.data.cartpage.sections || [];
                this.stypes = res.data.stypes;
                this.allProducts = res.data.products || [];
                this.allCategories = res.data.categories || [];
                this.allBanners = res.data.banners || [];
                this.allBrands = res.data.brands || [];
            } catch (e) {
                console.error("Failed to load cartpage", e)
            }
        },
        ...
        handleImageUpload(e, section) {
            ...
            if (!file) return;
            section.section_json.image_url = file;
            section.section_json.preview = URL.createObjectURL(file);
        },
        addSection(){
