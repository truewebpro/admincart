            ...
            <v-card>
              ...
              <v-card-text>
                  <v-form v-model="asvalid">
                      <v-text-field v-model="defaultItem.title" variant="outlined" density="compact" label="Cat Name"
...
<!--                                    :hint="domain+'/'+defaultItem.link" persistent-hint persistent-placeholder class="mb-2"></v-text-field>-->
                      ...
                      <v-btn :disabled="!asvalid" @click="addSearchCat" color="success" density="comfortable" class="mt-3">Submit</v-btn>
                  </v-form>
              </v-card-text>
            </v-card>
        </v-col>
            ...
            <v-card>
                <v-card-text>
                    <v-form v-model="usvalid">
                        <v-text-field v-model="editedItem.title" variant="outlined" density="compact" label="Cat Name" :rules="titleRule"></v-text-field>
...
<!--                                      :hint="domain+'/'+editedItem.link" persistent-hint></v-text-field>-->
                        ...
                        <div class="d-flex ga-3 mt-3">
                            <v-spacer/>
                            <v-btn :disabled="!usvalid" @click="updateSearchCat" color="success" density="comfortable">Update</v-btn>
        ...
        <v-dialog v-model="deleteDialog" max-width="300">
            <v-card>
                ...
                <v-card-actions>
                    <v-spacer/>
                    <v-btn @click="deleteSearchCat" color="success" density="comfortable" class="mt-3">Yes</v-btn>
...
export default {
    ...
    data(){
        return{
            ...
            titleRule:[
                ...
                v => !this.checkDuplicateCatName(v) || 'Cat name already exists',
            ]
        }
    },
    created() {
    ...
    methods:{
        getSearchCats(){
            axios.get('/sadmin/searchcats')
                .then((resp)=>{
                    this.scats = resp.data.searchcats;
                    this.alinks =  resp.data.cats;
                })
        },
        addSearchCat(){
            const adata = {
            ...
            axios.post('/sadmin/searchcat/update',adata)
                .then((resp)=>{
                    this.getSearchCats();
                    window.Toast.success("Cat Added Successfully");
        ...
        updateSearchCat(){
            const udata = {
                scat_id:this.editedItem.scat_id,
                title:this.editedItem.title,
                link:this.editedItem.link,
                status:this.editedItem.status,
            }
            axios.post('/sadmin/searchcat/update',udata)
                .then((resp)=>{
                    this.getSearchCats();
                    window.Toast.success("Cat Updated Successfully");
        ...
        deleteSearchCat(){
            ...
            axios.post('/sadmin/searchcat/update',ddata)
                .then((resp)=>{
                    this.getSearchCats();
        ...
        editItem(item){
            this.editedIndex = this.scats.indexOf(item);
        ...
        checkDuplicateCatName(name) {
            const nameLower = name.trim().toLowerCase();
            return this.scats.some((scat, index) => {
                const isSameName = scat.title.trim().toLowerCase() === nameLower;
