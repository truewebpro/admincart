<template>
    <v-container class="pa-1">
      <v-row dense>
          <v-col cols="12" md="12">
              <v-card>
                  <v-card-title>Search Brands</v-card-title>
              </v-card>
          </v-col>
        <v-col cols="12" md="8">
            <v-card>
                <v-data-table :items="sbrands" :headers="sbrandHeader" :hide-default-footer="sbrands.length < 20">
            ...
            <v-card>
              ...
              <v-card-text>
                  <v-form v-model="asvalid">
                      <v-text-field v-model="defaultItem.title" variant="outlined" density="compact" label="Brand Name"
...
<!--                                    :hint="domain+'/'+defaultItem.link" persistent-hint persistent-placeholder class="mb-2"></v-text-field>-->
                      ...
                      <v-btn :disabled="!asvalid" @click="addSearchBrand" color="success" density="comfortable" class="mt-3">Submit</v-btn>
                  </v-form>
              </v-card-text>
            </v-card>
        </v-col>
            ...
            <v-card>
                <v-card-text>
                    <v-form v-model="usvalid">
                        <v-text-field v-model="editedItem.title" variant="outlined" density="compact" label="Brand Name" :rules="titleRule"></v-text-field>
...
<!--                                      :hint="domain+'/'+editedItem.link" persistent-hint></v-text-field>-->
                        ...
                        <div class="d-flex ga-3 mt-3">
                            <v-spacer/>
                            <v-btn :disabled="!usvalid" @click="updateSearchBrand" color="success" density="comfortable">Update</v-btn>
        ...
        <v-dialog v-model="deleteDialog" max-width="300">
            <v-card>
                ...
                <v-card-actions>
                    <v-spacer/>
                    <v-btn @click="deleteSearchBrand" color="success" density="comfortable" class="mt-3">Yes</v-btn>
...
export default {
    ...
    methods:{
        getSearchBrands(){
            axios.get('/sadmin/searchbrands')
                .then((resp)=>{
                    this.sbrands = resp.data.searchbrands;
                    this.alinks =  resp.data.brands;
                })
        },
        addSearchBrand(){
            const adata = {
            ...
            axios.post('/sadmin/searchbrand/update',adata)
                .then((resp)=>{
                    this.getSearchBrands();
        ...
        updateSearchBrand(){
            const udata = {
                sbrand_id:this.editedItem.sbrand_id,
                title:this.editedItem.title,
                link:this.editedItem.link,
                status:this.editedItem.status,
            }
            axios.post('/sadmin/searchbrand/update',udata)
                .then((resp)=>{
                    this.getSearchBrands();
        ...
        deleteSearchBrand(){
            ...
            axios.post('/sadmin/searchbrand/update',ddata)
                .then((resp)=>{
                    this.getSearchBrands();
        ...
        editItem(item){
            this.editedIndex = this.sbrands.indexOf(item);
        ...
        checkDuplicateBrandName(name) {
            ...
            return this.sbrands.some((sbrand, index) => {
                const isSameName = sbrand.title.trim().toLowerCase() === nameLower;
